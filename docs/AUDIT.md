...
| AUDIT-067 | Color contrast `#6e7681` passes AA only for large text | 0.10.335 | (pending) | Surgical: all TEXT uses of `#6e7681` (the `--fwmon-text-faint` token + literal `color:` + `text-[#6e7681]` utilities; ~42 sites) retargeted to `#8b949e` (~5.8:1, AA). Distinct from AUDIT-066 `#768390` to keep faint tiers separate (avoids the v0.10.320 flatten revert). 2 decorative `border-color: var(--fwmon-text-faint)` pinned to literal `#6e7681`. `TestNoFaintTextColor_AUDIT067` walks the frontend tree. |
...
2026-06-06 — AUDIT-067 — surgical WCAG fix: #6e7681 text -> #8b949e (token+literal+utility; decorative untouched) — v0.10.335 — (pending) — opencode
