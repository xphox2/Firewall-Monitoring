...
| AUDIT-036 | DB pool sized for one process, not three daemons | 0.10.330 | (pending) | Pool size now per-process: api/poller/trap mains set 15/10/5 (DB_MAX_OPEN_CONNS overrides); NewDatabase reads cfg.Database.MaxOpenConns (fallback 25), idle capped at open limit. Was a flat SetMaxOpenConns(25) on each of 3 daemons = 75 conns/host. TestDBPoolPerProcess_AUDIT036. |
...
2026-06-06 — AUDIT-036 — per-process DB pool sizing (15/10/5, DB_MAX_OPEN_CONNS override) — v0.10.330 — (pending) — opencode
```
