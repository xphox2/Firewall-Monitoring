...
2026-06-08 — AUDIT-160 — vendored browser-lib provenance: VENDOR.md (file→lib→version→upstream→license, all MIT) + package.json vendoredBrowserLibraries pins; 5 versions read from banners, 3 fcose-stack flagged; shell guard — v0.10.404 — (pending) — claude
2026-06-08 — AUDIT-039 — WONTFIX (premise inverted): batchers consumed by syslog/trap-receiver/poller-PingCollector, api uses plural bulk saves; suggested fix would regress — v0.10.404 — (pending) — claude
2026-06-08 — AUDIT-126 — ACCEPT (premise stale): file now 1794 lines not 3740; per-tab split is cosmetic + risky churn — v0.10.404 — (pending) — claude
2026-06-08 — AUDIT-161 — ACCEPT-AND-DOCUMENT (finding's own option): sidebar duplication decision recorded in web/admin/README.md; templating = risky churn for a single-tenant tool — v0.10.404 — (pending) — claude
