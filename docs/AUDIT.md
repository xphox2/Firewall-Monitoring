...
| Metric | Value |
|---|---|
| Server version | **v0.10.399** |
| Bug findings resolved | **160 / 170  (94%)** |
| CRITICAL still open | **0** ✅ |
| Open bug findings | **10** |
...
- ⏳ Remaining 10: observability (150 OTel), the rest of test infra (123/140/142 — 123's harness already exists via 118), and smaller code item (094 entrypoint supervision, re-scope first) — all fully-local-verifiable
...
## ⏳ What's left (the 10 open findings)
...
- **Test infrastructure** — `AUDIT-123` (integration — note: the harness 123 asks for is already in place via 118), `AUDIT-140`/`142` (`t.Parallel`/`Short`). *(117 per-package coverage + 119 fuzz + 124 bench + 118 Postgres-CI-matrix + 120 property-based + 122 dead-test now done.)*
...
2026-06-08 — AUDIT-120 — property-based tests (stdlib testing/quick) for uptime % / spike stddev / counter-delta / FormatUptime math; invariants over every input, generators bounded to real domain; shell static guard — v0.10.398 — (pending) — claude
2026-06-08 — AUDIT-117 — first tests for the last zero-coverage critical-path packages: auth (lockout/bcrypt/JWT alg-confusion+wrong-secret+expiry+version/secure-token), alerts (policy helpers), models (TableName uniqueness + ToJSON); ping left to integration lane; fixed an in-progress token-roundtrip test bug — v0.10.399 — (pending) — claude
```
