...
## 📊 Status at a glance — updated 2026-06-08
...
- ✅ Container entrypoint fail-fast supervision — `wait -n` replaces the bare 3-PID wait so a single daemon crash triggers a clean full-stack restart instead of silent degradation — closes AUDIT-094 (Session 43)
- ✅ OpenTelemetry distributed tracing (OTLP/HTTP export, W3C probe→api propagation, trace_id in slog) — default-off, hand-rolled gin middleware + http transport to avoid a gin upgrade — closes AUDIT-150, **the last large item** (Session 44)
- ✅ Triaged the final 4 (039/126/160/161): 160 fixed (vendored-lib provenance doc + version pins), 039 wontfix (premise inverted — batchers ARE used by poller/trap-receiver/syslog, not the api), 126 & 161 accept-and-document (stale/cosmetic, risky churn) — **audit is 170/170, COMPLETE** (Session 45)
...
**All 170 bug findings are resolved (2026-06-08, v0.10.404).** Every theme is
cleared; the final 4 were adjudicated against current code in Session 45 (160
fixed with a provenance doc + version pins; 039 wontfix — premise inverted; 126
& 161 accepted-and-documented — stale/cosmetic). The 89 feature ideas (F01–F89)
remain out of scope for the hardening effort (future v0.11.0+).
...
- **Observability** — ✅ **theme cleared.** *(150 OpenTelemetry tracing + 076 slog + 077 Prometheus `/metrics` + 078 admin-action audit log + 129 client-error reporting + 135 request-ID all done.)*
...
- **Docs & repo hygiene** — ✅ **theme cleared**: 106/114/166/164/165 (Session 26) + 160 vendored-lib provenance (Session 45).
- **Smaller code cleanups** — ✅ **theme cleared.** *(094 entrypoint fail-fast supervision + 073 transport-type move + 071 JSONError helper + 081 `return err` wrapping + 129 client-error reporting + 132 ES5 `['catch']` sweep done.)*
- **Triage decisions** — ✅ 039 (wontfix, inverted premise), 126 & 161 (accept-and-document).
...
| AUDIT-160 | No vendored library version pinning | 0.10.404 | ab26e5c | Added `cmd/api/static/js/VENDOR.md` (provenance table: file → library → version → upstream → license; all MIT) + a `vendoredBrowserLibraries` pin block in `package.json`. Versions read from each artifact's embedded banner (Chart.js 4.4.7, chartjs-plugin-zoom 2.0.1, Cytoscape 3.30.4, GridStack 10.3.1, µPlot 1.6.31); the 3 webpack-bundled cytoscape-fcose-stack files carry no embedded version and are flagged to pin on next refresh (not guessed). Static guard `vendoring_audit160_test.go`. |
| AUDIT-039 | Per-daemon `BatchInserter` but only the api uses them | 0.10.404 | ab26e5c | **WONTFIX — premise inverted.** Verified the batched (singular) `SaveSyslogMessage`/`SaveTrapEvent`/`SavePingResult` are consumed by `internal/syslog`, `events.go` (trap-receiver), `internal/ping` (poller's PingCollector); the api handlers use the *plural* bulk saves and touch **zero** batchers. The suggested `NewDatabaseForAPI` fix would strip batching from the exact daemons that use it — a regression. Residue = a couple of idle batcher goroutines per daemon (sleeping ticker, no DB calls) = negligible. |
| AUDIT-126 | `admin-device-detail.js` 3740 lines / 213KB | 0.10.404 | ab26e5c | **ACCEPT — premise stale.** File is now 1794 lines / ~99KB (≈half the cited size). Splitting a working single-page detail IIFE per-tab is cosmetic maintainability with real regression risk and no functional gain (simplicity-first / minimal-impact). Revisit if it grows materially. |
| AUDIT-161 | Duplicate sidebar markup across admin HTML | 0.10.404 | ab26e5c | **ACCEPT-AND-DOCUMENT** (a resolution the finding itself offers). Single-tenant internal tool, small stable page set; Go-template/htmx-ifying the shell adds a render layer + regression risk to a working UI for a cosmetic DRY gain. Decision + "revisit if pages grow" trigger recorded in new `web/admin/README.md`. |
| AUDIT-150 | No OpenTelemetry tracing | 0.10.403 | 275be22 | New `internal/tracing` adopts `go.opentelemetry.io/otel`: OTLP/HTTP span export + W3C trace-context propagation across the probe→api hop. **OFF by default** (gated on `OTEL_TRACES_ENABLED`; zero init/goroutines/overhead disabled). api installs a hand-rolled gin server-span middleware (`GinMiddleware`, route-template-named, extracts inbound context); probe relay wraps its HTTP transport (`WrapTransport` → client spans + `traceparent` inject); AUDIT-076 slog handler stamps `trace_id`/`span_id` on records logged under an active span (via the request ctx `RequestLogger` already passes) for log↔trace correlation. **Hand-rolled the gin/http instrumentation on the otel API instead of the `otelgin`/`otelhttp` contrib** — those force gin 1.10→1.12 + a big transitive tree (quic-go/mongo/grpc-gateway); lean path keeps gin pinned, adds only otel core/SDK + OTLP-HTTP exporter. Std `OTEL_EXPORTER_OTLP_*` config (config.env.example). Tested without a collector: in-memory recorder proves cross-process propagation (client+server share one trace ID) + disabled no-ops + trace-ID log stamping. Static guard `tracing_audit150_test.go`. **The last Large item.** |
| AUDIT-094 | `entrypoint.sh` runs 3 binaries with bare `wait` | 0.10.402 | e148078 | The signal-trap half (the S-scope) was already in place; this adds the real value. Replaced the final `wait $API_PID $POLLER_PID $TRAP_PID` (blocks until ALL exit → a crashed daemon leaves a silently-degraded stack) with a **`wait -n` fail-fast**: on the first daemon exit, log which one + status, `teardown()` (daemons then PG), `exit 1` → compose `restart: unless-stopped` rebuilds a complete stack. Factored shared stop into `teardown()`; `\|\| true`-guarded its kill/wait/pg_ctl so `set -e` can't leak 143 into a clean `docker stop` (exit 0) or skip the crash-path `exit 1`. Chose `wait -n` over s6/supervisord (no new image dep; clean-slate restart; reviewable + unit-testable without a live container). Verified via mock harness (crash→exit 1, SIGTERM→exit 0) + `bash -n`; `wait -n` needs bash≥4.3 (Alpine 3.19 = bash 5.2). Static guard `entrypoint_supervision_audit094_test.go`. **⚠ Refined in v0.10.405 (commit `_pending_`):** the `wait -n` "tear down on ANY daemon exit" form **regressed prod** — the trap-receiver legitimately refuses to start without `SNMP_TRAP_COMMUNITY` (AUDIT-012), so on a deployment without one it exited immediately and forever and the fail-fast crash-looped the whole container (web UI never came online). Now supervises the **essential api only** (`wait "$API_PID"`): poller/trap death leaves the api/UI up (degraded, visible), only api exit tears down. Paired with `TrapReceiver.Enabled()` → trap-receiver **idles** (no open listener) instead of fatal-exiting when no community is set, so no trap config is required to boot. Mock-harness verified (non-essential death doesn't tear down; api exit→exit 1; SIGTERM→exit 0). |
| AUDIT-123 | No integration tests | 0.10.401 | 52465d0 | The tag/`make test-integration`/CI-Postgres-service triad was already delivered by AUDIT-118 (v0.10.379), but 118's suite tests the `database` package's PG paths in isolation. Added the missing cross-package, full-stack piece: `internal/api/handlers/integration_pg_test.go` (`TestEndToEndIngestion_Postgres_AUDIT123`) drives a real HTTP request → gin handler → probe Bearer auth → `SaveSystemStatus` → real partitioned Postgres, then asserts persistence, **current-month partition routing** (`system_status_YYYYMM`), the device→online handler side-effect, and a query-layer round-trip. Promoted 118's private `newPGForTest`/`cfgFromDSN` to a shared exported `database.NewIntegrationDB(testing.TB)` (`integration_testkit.go`); the 118 suite now delegates to it. Broadened the CI `integration-postgres` job + `make test-integration` from `./internal/database/...` to also run `./internal/api/handlers/...`. Verified locally via compile-under-tag + skip-without-DSN (real run = CI). Static guard `integrationtests_audit123_test.go`. |
...
2026-06-06 — AUDIT-106 — README endpoint sweep (~170 routes grouped) + who-is-this-for/when-not/comparison sections — v0.10.369 — afac8ad — opencode
...
2026-06-08 — AUDIT-076 — structured logging via log/slog: slog.SetDefault bridges all ~460 legacy log.Printf at once + native-record chokepoints (InternalError/RequestLogger) + secret-name redaction + LOG_FORMAT/LOG_LEVEL env — v0.10.396 — 2932241 — claude
2026-06-08 — AUDIT-073 — move HTTP transport envelope (APIResponse + constructors) out of internal/models into new internal/api/response leaf package; 435 sites/16 files, JSON shape unchanged; + gofmt CI-gate fix for the two 076 test files — v0.10.397 — af0e756 — claude
2026-06-08 — AUDIT-120 — property-based tests (stdlib testing/quick) for uptime % / spike stddev / counter-delta / FormatUptime math; invariants over every input, generators bounded to real domain; shell static guard — v0.10.398 — cfc3a3f — claude
...
2026-06-08 — AUDIT-123 — end-to-end handler→real-Postgres integration test (HTTP → probe auth → SaveSystemStatus → partition routing → query-back); shared exported database.NewIntegrationDB harness; CI+Makefile broadened to handlers pkg; shell guard — v0.10.401 — 52465d0 — claude
2026-06-08 — AUDIT-094 — entrypoint.sh fail-fast supervision: wait -n replaces the bare 3-PID wait (silent degradation), crash → teardown + exit 1 → full-stack restart; teardown() helper with || true guards; mock-harness + bash -n verified; shell guard — v0.10.402 — e148078 — claude
2026-06-08 — AUDIT-150 — OpenTelemetry tracing (internal/tracing): OTLP/HTTP export + W3C probe→api propagation + trace_id/span_id in slog; default-off; hand-rolled gin middleware + http transport (no contrib gin upgrade); cross-process propagation proven via in-memory recorder; shell guard. THE LAST LARGE ITEM — v0.10.403 — 275be22 — claude
2026-06-08 — AUDIT-160 — vendored browser-lib provenance: VENDOR.md (file→lib→version→upstream→license, all MIT) + package.json vendoredBrowserLibraries pins; 5 versions read from banners, 3 fcose-stack flagged; shell guard — v0.10.404 — ab26e5c — claude
2026-06-08 — AUDIT-039 — WONTFIX (premise inverted): batchers consumed by syslog/trap-receiver/poller-PingCollector, api uses plural bulk saves; suggested fix would regress — v0.10.404 — ab26e5c — claude
2026-06-08 — AUDIT-126 — ACCEPT (premise stale): file now 1794 lines not 3740; per-tab split is cosmetic + risky churn — v0.10.404 — ab26e5c — claude
2026-06-08 — AUDIT-161 — ACCEPT-AND-DOCUMENT (finding's own option): sidebar duplication decision recorded in web/admin/README.md; templating = risky churn for a single-tenant tool — v0.10.404 — ab26e5c — claude
2026-06-08 — 🎉 AUDIT COMPLETE: 170/170 bug findings resolved (0 open, 0 CRITICAL) at v0.10.404 — claude
2026-06-09 — AUDIT-094/012 follow-up (prod fix) — fixed a v0.10.402 crash-loop regression: entrypoint now supervises the essential api only (`wait "$API_PID"`, not `wait -n`) so a poller/trap death doesn't kill the stack; trap-receiver idles (TrapReceiver.Enabled()) instead of fatal-exiting when no SNMP_TRAP_COMMUNITY → server boots with no trap config required. Mock-harness + unit tested — v0.10.405 — (pending) — claude
```
...
- **AUDIT-094 — RESOLVED in v0.10.402 (Session 43):** the dedicated pass chose
  the `wait -n` fail-fast loop (not s6/supervisord) — a crashed daemon now tears
  the stack down and `exit 1`s so the restart policy rebuilds a complete stack,
  instead of the bare 3-PID `wait` leaving a silently-degraded container. The
  signal-trap S-scope was already in place. (This note kept for history; see the
  Resolved table for the authoritative status.)
...
| AUDIT-164 | 0.10.372 | a5026aa | FUNDING accept/wontfix — not soliciting donations, no `FUNDING.yml` shipped; decision documented. |
...
## Session 28 completion log (2026-06-06) — observability: admin-action audit log

**1 audit shipped** (v0.10.374), resolved count **146 → 147**. New package
`internal/audit`, new `models.AuditLog` table, new read endpoint. Full suite
green.
...
- **Registration order is load-bearing and now test-pinned.** The middleware
  must sit AFTER `AdminAuth` (so `username`/`user_id` are on the context) and
  after CSRF (so forged/rejected requests aren't recorded as "actions").
  `TestAuditWiring_AUDIT078` asserts the source order, not just presence.
- **It records the *outcome*, not just the attempt.** Because it runs
  post-`c.Next()`, a mutation that 500s or 403s is still recorded with that
  status — the incident-investigation case the audit cared about. GETs are
  filtered out (read-only + far too high-volume).
- **`setupRoutes` needed a `db` param.** Route registration lives in a separate
  function from `main()`, so `db` wasn't in scope — threaded it through rather
  than exposing `handler.db`. Future middleware that needs the DB at the route
  layer has the seam now.
- **Cardinality discipline matches the metrics work:** `Action` stores the route
  *template* (`/admin/api/devices/:id`), `Target` stores the concrete params
  (`id=5`) — so you can filter "all device deletes" without one action-string
  per id. Same lesson as AUDIT-077.
- **Three honest deferrals:** (1) no `/admin/audit` **UI page** — the SPA view is
  follow-up frontend work; the API is the deliverable; (2) no **before/after
  field diffing** — needs per-handler cooperation to capture prior state; (3)
  **no auto-pruning** — admin mutations are very low-volume, so unbounded growth
  is a non-issue for years; revisit only if a high-frequency mutating endpoint
  appears.
- **Remaining 23 are the hard core.** Observability now has only **076 (slog)**
  and **150 (OTel)** left; 076 is still the highest-leverage single item and the
  three logging seams to thread it through are now all in place (071's
  `InternalError`, 077's metrics middleware, 078's audit middleware). It remains
  a ~150-`log.Printf` change that wants a planning pass — a good candidate for a
...
| Audit | Version | Code commit | What shipped |
|---|---|---|---|
| AUDIT-118 | 0.10.379 | d39e80f | Build-tagged Postgres integration suite (`TEST_PG_DSN`) + CI `integration-postgres` job (`postgres:16` service) + `make test-integration`. |
