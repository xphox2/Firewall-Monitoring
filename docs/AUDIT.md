...
| AUDIT-042 | Relay client has no idempotency key | 0.10.327 | (pending) | Collector sends a stable `X-Probe-Batch-ID` (random 128-bit hex, reused across a batch's retries); server records `(probe_id, batch_id)` in a new `processed_batches` table and dedupes repeat requests, recording the key only after a 2xx save (a failed save isn't dedupe-dropped on retry). Applied to `/syslog` `/traps` `/flows` `/pings`; 2-day cleanup retention. Collector v1.2.74. Tests: `handlers_data_idempotency_audit042_test.go` (server), `relay_idempotency_audit042_test.go` (collector). Deferred: direct-send endpoints. |
...
2026-06-05 — AUDIT-042 — X-Probe-Batch-ID idempotency (collector v1.2.74 + server processed_batches dedup) — v0.10.327 — (pending) — opencode
```
