...
| AUDIT-016 | Probe registration key compared with `!=` not `hmac.Equal` | 0.10.246 | (pending) | `validateProbe` (`handlers_probes.go:606`) now uses `subtle.ConstantTimeCompare` for the post-PK-lookup token check. `authenticateProbeByBearer`'s SQL `WHERE registration_key = ?` lookup is a separate timing channel that needs the AUDIT-017 hashed-token refactor; this fix covers the high-leverage path used by 18 ingestion handlers. 9-case regression test in `handlers_probes_audit016_test.go`. |
...
2026-06-02 — AUDIT-016 — probe key constant-time compare in validateProbe — v0.10.246 — (pending) — opencode
```
