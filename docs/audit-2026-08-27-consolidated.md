...
The single refuted finding is AUDIT-198 (dead/unreachable dashboard code). The 13 unresolved findings are the two remediation batches that were never executed: Batch 6 (AUDIT-176, the sole HIGH still open — serverhealth test coverage) and Batch 9 (the collector ingest-attribution/hardening set: AUDIT-186, 187, 216, 237, 263, 282, 283, 284, 305, 306, 307, 317). See each inline blockquote for detail.
...
> **⚠️ DISPOSITION UNRESOLVED** — HIGH server test-gap — planned as Batch 6 (serverhealth tests) but never executed: no test-coverage commit, CHANGELOG entry, or git history for the DataDirLocator crash-loop fallback / dataOK strictness invariants in either repo.
...
> **⚠️ DISPOSITION UNRESOLVED** — MEDIUM collector data-integrity — planned as Batch 9 (STRICT source binding, user-approved) but never executed: sFlow agent_address is still not bound to the UDP source; no fix in the collector CHANGELOG or git history.
...
> **⚠️ DISPOSITION UNRESOLVED** — MEDIUM collector security — planned as Batch 9 (STRICT source binding, user-approved) but never executed: TFTP WRQ device-ID is still unbound from the client address; no fix shipped.
...
> **⚠️ DISPOSITION UNRESOLVED** — MEDIUM collector input-hardening — planned as Batch 9 but never executed: the unrecognized-trap path still logs one line per varbind (log-flood amplification); no fix shipped.
...
> **⚠️ DISPOSITION UNRESOLVED** — LOW collector data-integrity — planned as Batch 9 but never executed: ifaceIPMap is still never pruned on device-list refresh; no fix shipped.
...
> **⚠️ DISPOSITION UNRESOLVED** — LOW collector input-hardening — planned as Batch 9 but never executed: parseBool still silently maps unrecognized values to false; no fix shipped.
...
> **⚠️ DISPOSITION UNRESOLVED** — LOW collector input-hardening — planned as Batch 9 but never executed: the IPFIX field-spec parser still pre-allocates from an unvalidated attacker-controlled count; no fix shipped.
...
> **⚠️ DISPOSITION UNRESOLVED** — LOW collector input-hardening — planned as Batch 9 but never executed: seqTracker still caps but never evicts its state map; no fix shipped.
...
> **⚠️ DISPOSITION UNRESOLVED** — LOW collector input-hardening — planned as Batch 9 but never executed: a v9 template field with Length 0xFFFF still bypasses the field-width quarantine; no fix shipped.
...
> **⚠️ DISPOSITION UNRESOLVED** — LOW collector input-hardening — planned as Batch 9 but never executed: UDP syslog still logs one line per malformed datagram; no fix shipped.
...
> **⚠️ DISPOSITION UNRESOLVED** — LOW collector concurrency — planned as Batch 9 but never executed: the TFTP writeHandler still runs on an untracked goroutine (Shutdown races an in-flight upload); no fix shipped.
...
> **⚠️ DISPOSITION UNRESOLVED** — LOW collector input-hardening — planned as Batch 9 but never executed: TFTP handleRRQ still bypasses the AUDIT-050 source-IP allowlist / rate limit; no fix shipped.
...
> **⚠️ DISPOSITION UNRESOLVED** — INFO collector security — planned as Batch 9 but never executed: the collector trap community check still uses a non-constant-time comparison; no fix shipped.
