...
Left out of AUDIT-320's scope on purpose, and the distinction is about the COLLECTOR's interface, not the server's. `GetAllVPNTunnels` was on the server's `VendorProfile`, but it has **no counterpart in the collector at all**, so deleting it converged the two repos. These three do have live collector counterparts: `ParseVPNStatus` on the collector's `VendorProfile` (`internal/snmp/vendor.go:35`), and `GetVPNStatus`/`GetProcessorStats`/`GetInterfaceAddresses` as concrete methods on the collector's `*snmp.SNMPClient` (`internal/snmp/snmp.go:534`, `:626`, `:674`), all three called from `pollDevice` (`cmd/collector/main.go:1953`, `:1965`, `:1938`). Deleting the server's copies would therefore diverge the repos. That makes it a design decision rather than dead-code removal. (`GetInterfaceAddresses` is vendor-neutral in both repos: it walks `ipAddrTable` directly and resolves no vendor profile.) Note also that the deleted walker was the only code stamping `TunnelType = "ipsec"` on FortiGate site-to-site results, so anything reviving `GetVPNStatus` would get an empty `TunnelType` — a pre-existing gap in `ParseVPNStatus`, not a regression.

> **OPEN** — two decisions, not one. (a) `GetVPNStatus` and `GetProcessorStats` ride the `VendorProfile` vendor-extension contract (`ParseVPNStatus`, `ParseProcessors`): decide whether the server keeps that contract at all. (b) `GetInterfaceAddresses` is vendor-neutral — it walks `ipAddrTable` and resolves no profile — so answering (a) does NOT dispose of it; it needs its own call.
