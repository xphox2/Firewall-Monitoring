...
- **P2-6 Publish GitHub releases/tags for shipped versions** *(nice-to-have, noted 2026-08-28)* — every release today is only a `ServerVersion` bump in `cmd/api/main.go` plus a CHANGELOG entry; the GitHub repo has **zero tags and zero releases**. The technicallabs.org System Monitor resolves each project's live version from the GitHub API (`releases/latest`, falling back to `tags`), so it can never resolve this repo and permanently shows its stale hardcoded baseline (v0.11.122 while the server is on 0.11.212) no matter how many versions ship. The stale README version badge (0.10.553) is the same missing-surface symptom. Would-be-nice fix: a small CI step on master that tags `v<ServerVersion>` (and optionally `gh release create`) whenever the version constant changes. The same gap exists in Firewall-Collector (see its `docs/FEATURES.md` roadmap).
