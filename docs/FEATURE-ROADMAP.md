...
| Syslog receiver (RFC 5424 + RFC 3164, TCP+UDP/514) | Collector | Stable | Collector binds 514 and parses RFC 5424 (6 timestamp formats) + RFC 3164 + FortiGate hostname/SD extraction + source allow-list, then relays; the server has no raw syslog listener and only stores the relayed `syslog_messages`/`syslog_summaries`. |
| sFlow v5 parser (UDP/6343) | Collector | Stable | Collector binds 6343 and parses Ethernet + 802.1Q VLAN, IPv4/IPv6, TCP/UDP (fuzz target), then relays; the server has no raw sFlow listener and only stores the relayed `flow_samples`. |
| ICMP ping / reachability | Collector | Stable | Collector runs raw net/icmp (latency+loss, 10-concurrent semaphore, needs `NET_RAW`) and relays results; the server runs no ICMP prober of its own. |
