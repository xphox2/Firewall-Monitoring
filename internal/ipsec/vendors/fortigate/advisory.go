...
// routes Render installs. routeSteps deliberately omits `distance`, so the
// device applies its default of 10; TestRenderTunnelRouteOmitsDistance pins that
// omission so this constant cannot drift away from what Render actually does.
...
// protected subnet away from this tunnel. A deploy adds one route per remote
// subnet via the VTI at the FortiOS default distance; an existing route for an
// overlapping prefix at a LOWER distance wins outright (traffic never enters the
// tunnel), and one at an EQUAL distance is installed alongside ours and
// ECMP-balanced (roughly half the traffic leaves the wrong way). Both are silent
// at deploy time — the tunnel comes up and simply does not carry the traffic.
//
...
func (d driver) Advisories(v ipsec.RenderView, bodies map[string]string) []ipsec.Advisory {
	...
	for _, r := range rows {
		...
		if !hasDist {
			dist = fgTunnelRouteDistance
		}
		if dist > fgTunnelRouteDistance {
			continue // our tunnel route wins on distance — no advisory
		}
		for _, w := range wanted {
			...
			out = append(out, fgRouteAdvisory(in.Name, w.text, dstText, dst, r, seq, hasSeq, dist))
			break
		}
	}
...
func fgRouteAdvisory(tunnel, wanted, dstText string, dst *net.IPNet, r map[string]any, seq int, hasSeq bool, dist int) ipsec.Advisory {
	via := fgStr(r, "gateway")
	dev := fgStr(r, "device")
	switch {
	case via != "" && dev != "":
	...
	if dist < fgTunnelRouteDistance {
		...
		a.Detail = fmt.Sprintf(
			"%s covers %s (%s) via %s at distance %d. The route %s installs for %s uses the FortiOS default distance %d, so the existing route stays preferred and traffic to %s will never enter the tunnel — the tunnel will come up but carry nothing for this prefix.",
			id, prefixText(dst, dstText), wanted, via, dist, tunnel, wanted, fgTunnelRouteDistance, wanted)
	} else {
		a.Title = fmt.Sprintf("a pre-existing static route to %s ties with this tunnel's route", wanted)
		a.Detail = fmt.Sprintf(
			"%s covers %s (%s) via %s at distance %d — the same distance the route %s installs for %s. FortiOS installs both and ECMP-balances between them, so roughly half the traffic to %s leaves via %s instead of the tunnel.",
			id, prefixText(dst, dstText), wanted, via, dist, tunnel, wanted, wanted, via)
	}
	a.Remedy = fmt.Sprintf(
		...
		fgTunnelRouteDistance, seqText(seq, hasSeq), fgTunnelRouteDistance+10)
	return a
}
