...
func (h *Handler) GetDeviceInterfaceErrorsLatest(c *gin.Context) {
	if h.db == nil {
		c.JSON(http.StatusServiceUnavailable, models.ErrorResponse("Database not available"))
		return
	}
	...
	var latest []models.InterfaceErrors
	subQuery := h.db.Gorm().Model(&models.InterfaceErrors{}).
		Select("interface, MAX(timestamp) as max_ts").
		Where("device_id = ?", uint(id)).
		Group("interface")

	if err := h.db.Gorm().Model(&models.InterfaceErrors{}).
		Joins("INNER JOIN (?) as latest ON interface_errors.interface = latest.interface AND interface_errors.timestamp = latest.max_ts", subQuery).
		Where("interface_errors.device_id = ?", uint(id)).
		Find(&latest).Error; err != nil {
		c.JSON(http.StatusInternalServerError, models.ErrorResponse("Failed to get latest interface errors"))
		return
	}
	c.JSON(http.StatusOK, models.SuccessResponse(gin.H{"interface_errors": latest}))
}
