...
// normalizeTopologyMAC lowercases relayed MAC/chassis fields defensively.
// The collector already sends canonical lowercase; the inference joins these
// against interface MACs case-sensitively, so enforce it at the boundary.
func normalizeTopologyMAC(s string) string { return strings.ToLower(strings.TrimSpace(s)) }
...
func (h *Handler) ReceiveTopologyEntries(c *gin.Context) {
	...
	for i := range entries {
		if entries[i].DeviceID > 0 && allowedDevices != nil && !allowedDevices[entries[i].DeviceID] {
...
func (h *Handler) ReceiveTopologyNeighbors(c *gin.Context) {
	...
	for i := range neighbors {
		if neighbors[i].DeviceID > 0 && allowedDevices != nil && !allowedDevices[neighbors[i].DeviceID] {
			continue
		}
		if neighbors[i].Protocol != "lldp" && neighbors[i].Protocol != "cdp" {
			continue
		}
		if neighbors[i].Timestamp.IsZero() {
			neighbors[i].Timestamp = now
		}
		neighbors[i].ID = 0
