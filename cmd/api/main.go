...
func setupRoutes(router *gin.Engine, cfg *config.Config, handler *handlers.Handler, authManager *auth.AuthManager, db *database.Database) {
	...
	admin.Use(middleware.RequireRole(
		...
		map[string]bool{ // adminOnlyRoutes — role=admin, any method
			...
			// Preflight + deploy saga touch device credentials (API token) and WRITE
	...
	))
	{
		...
		// Deploy saga (C2b-1: WRITES config to the FortiGate end). Deploy renders +
		// enqueues per-end apply commands; GET polls status; rollback reverses from
		// the stored remove snapshot. All admin-only (in adminOnlyRoutes).
		...
		admin.GET("/api/settings/syslog-retention", handler.GetSyslogRetention)
