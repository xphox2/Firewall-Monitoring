...
(function() {
    ...
    async function loadPanelTrafficChart(connId, range) {
        try {
            ...
            if (!Array.isArray(data) || data.length === 0) {
                canvas.parentElement.innerHTML = '<div style="text-align:center;color:#8b949e;padding:30px;">No traffic data available for this connection.</div>';
                return;
            }
    ...
    async function loadPanelFlowStats(connId, hours) {
        try {
            ...
            if (!hasData) {
                content.innerHTML = '<div style="text-align:center;color:#8b949e;padding:30px;">No sFlow data available for this connection.</div>';
                return;
            }
    ...
    function renderPanelTunnelTable(tableId, tunnels, deviceId) {
        ...
        if (!tunnels.length) {
            tbody.innerHTML = '<tr><td colspan="6" style="text-align:center;color:#8b949e;padding:16px;">No tunnels</td></tr>';
            return;
        }
        let html = '';
        tunnels.forEach((t, i) => {
    ...
    function renderPanelPhase2(matches, srcName, dstName) {
        ...
        if (!matches.length) {
            container.innerHTML = '<div style="text-align:center;color:#8b949e;padding:20px;">No Phase 2 selector matches.</div>';
            return;
        }
        let html = '';
        matches.forEach((m, i) => {
    ...
    function showRichVPNDetailPanel(deviceId, offnetOnly, devices, vpnMap) {
        ...
        function renderVPNTunnelRows(prefix, rows, devId) {
            if (!rows.length) return '<tr><td colspan="9" style="text-align:center;color:#8b949e;padding:12px;">None</td></tr>';
            return rows.map((t, i) => {
        ...
        if (!offnetOnly && matched.length === 0 && offnet.length === 0) {
            sectionsHtml = '<div style="text-align:center;color:#8b949e;padding:20px;">No tunnels</div>';
        }
