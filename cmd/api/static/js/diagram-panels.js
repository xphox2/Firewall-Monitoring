...
(function() {
    ...
    async function showRichConnDetailPanel(conn) {
        ...
        // AUDIT-184: the unknown-method fallback renders the raw match_method —
        // escape it (the known labels are literals, but escaping both arms is
        // harmless and keeps the template uniform).
        const methodBadge = conn.match_method ? `<span class="badge info" style="font-size:0.68rem;">${window.escapeHtml(methodLabels[conn.match_method] || conn.match_method)}</span>` : '';
