...
(function () {
    ...
    function onDevicesChosen() {
        ...
        AC.apiFetch(API + '/ipsec/capabilities?a=' + encodeURIComponent(vendorOf(a)) + '&b=' + encodeURIComponent(vendorOf(b))).then(function (r) {
            caps = (r && r.data) || null;
            if (!caps) return;
    ...
    function collectIntent() {
        ...
        function end(dev, pfx) {
            return {
                ...
                child_lifetime_secs: childLife, mss_clamp: 1350
            };
        }
        return {
    ...
    function loadTunnelIntoWizard(id) {
        AC.apiFetch(API + '/ipsec/tunnels/' + id).then(function (r) {
            ...
            // PSK stays masked (unchanged on save).
