...
### Added
- **Server-side per-tenant probe authorization** (AUDIT-067, v0.10.363): the collector's bearer token (`PROBE_REGISTRATION_KEY`) is now scoped to a single tenant per probe. A new `tenant_id` column on `probes` (default `"default"`, backfilled on existing rows so the v0.10.363 cross-tenant 403 is a no-op for pre-existing single-tenant deployments) identifies the owning tenant, and the auth path now does a *two-probe* lookup: it resolves the presented token to its owning probe, then refuses with **403 Cross-tenant access denied** when the URL `:id` probe's `tenant_id` differs from the token-owning probe's. The pre-existing 401 (wrong key) and 200 (correct key) paths are unchanged — the new 403 is a third, semantically distinct outcome for the cross-tenant attempt. Operator setting of `tenant_id` happens through the same `CreateProbe` payload path as every other field; splitting two probes into different tenants is a deliberate, explicit action. **`probe_token_audits` table** records every authenticated request (`tenant_id`, `probe_id`, `endpoint`, `method`, `remote_ip`, `status_code`, `timestamp`) — both successes AND failures (so a stolen token's brute-force attempts show up in the timeline, not just its final successful use). All four handler paths are wired: `validateProbe` (the URL-bound probe endpoints), `authenticateProbeByBearer` (the `/heartbeat` endpoint), `RegisterProbe` (the registration handshake), and the new `probeAuthError` helper used by both. **Deferred (follow-up issues, flagged here so a future PR knows):** (1) **IP allowlist per probe** — the audit log already records `remote_ip` for forensic review, but a tenant admin cannot yet *gate* on it; the schema-level addition is one `probe_ip_allowlist` table + a 403-emitting check in the same auth helpers, tracked as a separate audit. (2) **Short-lived JWT migration** — long-lived bearer tokens are unchanged in v0.10.363; the move to ~24h JWTs with refresh-on-heartbeat is a follow-up that lands with the broader admin-JWT story. (3) **24h rotation grace period** — `RegenerateProbeKey` (AUDIT-085) is already atomic; the additional "old key valid for 24h after rotation" mechanic for zero-downtime rollouts is also deferred (the collector's `tryReregister` path on 401 already handles immediate-revocation correctly). The four new tests in `handlers_probes_audit067_test.go` pin the *current* contract for all three deferred items, with TODO-style comments naming exactly what assertion each will gain when the follow-up work ships.
