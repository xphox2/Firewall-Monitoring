...
### Added
- **Server-side X-Probe-Batch-ID handling, end-to-end** (AUDIT-070, v0.10.363): the collector's stable per-batch `X-Probe-Batch-ID` header (AUDIT-042, collector v1.2.74) was being recorded by the server but the response gave the collector no machine-readable way to know it had been deduped, and operators had no visibility into how often dedup actually fired. Four additions: (1) every probe batch endpoint now sets `X-Probe-Server-Batch-Status: new|duplicate` on the response — the collector can short-circuit retry accounting on `duplicate` without parsing the body; (2) a `probe_batch_dedup_total{endpoint="syslog|traps|flows|pings"}` Prometheus counter, exposed at `GET /api/metrics` in standard text-exposition format, so the operator can graph "how flaky is the central server" directly; (3) a `WARN: Duplicate batch X from probe Y (endpoint Z, ttl=...), dropped.` log line on every dedup hit — a spike is an immediate signal that the central server is timing out and the collector is retrying; (4) a configurable `BATCH_DEDUP_TTL` (default 5 minutes) so a retry that arrives after the collector's retry window has fully closed re-ingests the batch instead of being silently dropped. The TTL is applied at read time (`timestamp >= now - ttl` in `BatchAlreadyProcessed`), the 1-day cleanup retention is a backstop to bound the table, and the zero-TTL misconfig falls through to the 5-minute default in two places (handler + database) so a buggy config can't disable dedup or dedupe-forever. New `internal/metrics` package is a hand-rolled atomic-counter + text-format exporter — no `prometheus/client_golang` dependency was added because the four metrics the server actually needs fit in <300 lines and the dep would drag in protobuf + a half-dozen `x/*` upgrades. Tests: 9 new (4 handler integration, 4 metrics unit, 1 default-TTL guardrail), 0 changed. Storage note: the dedup cache is the existing `processed_batches` Postgres/SQLite table, so dedup is multi-instance safe (any pod handles any probe's retry); the metric is in-process per pod, which matches the standard Prometheus model (scraper sums across instances).
