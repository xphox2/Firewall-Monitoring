...
- **`#484f58` text contrast fixed (WCAG AA)** (AUDIT-066): `#484f58` on the dark UI backgrounds is ~2.6:1 — it fails WCAG AA even for large text, yet it was used as readable label/placeholder/empty-state text. Brightened every **foreground-text** use of it to `#8b949e` (≈5.3:1) across the source CSS, the bundled `tailwind.css`, the admin/public HTML, and the JS that builds inline-styled HTML. Decorative uses are deliberately left alone: Chart.js axis-tick colors (always the quoted `'#484f58'` form) and `border-color`/`background` stay dark. The transform — an equal-length hex swap scoped to the `color:` property and `text-[…]` utilities (never `*-color:` or quoted chart values) — is in `scripts/audit_brighten_color.py`. `TestColorContrast_NoDarkText484_AUDIT066` in `internal/shell/` pins that no foreground-text `#484f58` remains.
- **`#6e7681` text contrast lifted (WCAG AA for small text)** (AUDIT-067): `#6e7681` on the dark backgrounds is ≈4.07:1 — it passes WCAG AA only for *large* text, but it was the color of small stat labels and muted/secondary text throughout the admin + public UI. Lifted every foreground-text use to `#8b949e` (≈5.3:1), so small text now passes AA. Same scoped, equal-length swap as AUDIT-066 (`color:` property + `text-[…]` utilities only; chart ticks and borders untouched), via `scripts/audit_brighten_color.py`. `TestColorContrast_NoDarkText6e_AUDIT067` in `internal/shell/` pins that no foreground-text `#6e7681` remains.
