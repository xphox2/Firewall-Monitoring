...
- **`ddos_prefix` (SFLOW_DDOS_PREFIX, critical default).** Carpet-bombing detection: candidate victims folded into fixed /24 (IPv4) and /64 (IPv6) prefixes in Go, fired at 0.75× the prefix threshold (bounded-undercount estimator, recorded in details), suppressed when a single host already fired per-host.
- **Victim-keyed routing.** Victim-keyed security detections bypass the per-source SFLOW_SECURITY consolidation (their subject is the victim, not an offending source) and route down the per-detection path — while flow_security suppress Event Rules now run over ALL security detections first, with the rule's source_ip matching the victim address for these, keeping Event Rules the single silencing surface.
...
- **3-consecutive-cycle debounce** absorbs poller-restart-after-outage storms, collector spool replay (old rows bumping `last_polled` to now), and re-enabled devices' first cycle (streaks are pruned when a device leaves the eligible set, so a carried-over count can't defeat the debounce).
- **Query-failure freeze**: a cycle where either signal query errored neither fires nor recovers — without this, one transient interface-query failure would send a false "recovered" notification for an alert held open by the interface signal, then re-page when the query healed.
...
### Added
- **Site filter on the Alert History page.** New Site dropdown + `?site_id=` query support in `applyAlertFilters`, scoping to a site via device→site and folding in site-scoped alerts (e.g. `SFLOW_SECURITY_DIGEST`) through the `alerts.site_id` column. Understands a special `site_id=unassigned` bucket. Covered by `handlers_alerts_site_filter_test.go`.

### Fixed
- **Alert History `device_id` deep-links now actually filter.** The `alerts-filter-device` dropdown was never populated with options, and `buildAlertParams` read the empty `<select>` value — so `/admin/alerts?device_id=42` showed a filter chip but returned unfiltered results. `buildAlertParams` now sources filters from page state, the device/site dropdowns are populated, and the SPA click-interceptor re-seeds analytics filters from the URL when switching pages (not just when already on the page).
- **NOC "Live Detections" feed was almost always blank.** It queried only the last **15 minutes** (excluding detections already promoted to alerts), and un-alerted detections average ~4/hour, so ~90% of windows were empty. Widened to a **6-hour** window (limit 30). The feed also gains **Source → Dest** and **Device** columns (data already in the payload; source IPs link into Threat Intel).
