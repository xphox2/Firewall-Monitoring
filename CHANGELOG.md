...
### Added
- **Server-side accept + surface `ConfigRevision.BackupQuality`** (AUDIT-069, v0.10.363): the collector added `backup_quality` (`full` / `masked` / `suspect` / `unknown`) and `trigger_source` (`syslog` / `poll` / `manual`) to its `ConfigRevision` wire payload in v1.2.72, and the server's `DeviceConfigRevision` struct already has both fields and persists them, but the API response always emitted the keys (even when empty) and the per-device history endpoint accepted no quality filter. Added `omitempty` to both JSON tags so legacy responses stay clean, a `?quality=masked|full|suspect|unknown` filter on `GET /api/devices/:id/config-history` so operators can scan "which of my backups are masked and therefore not restorable as-is?", and an in-process atomic counter per quality label (`probe_config_backup_quality_total{quality=…}`) incremented in `ReceiveConfigRevision` so the eventual Prometheus endpoint (AUDIT-077) can just iterate and emit, and tests can assert the value moves. `TestConfigRevision_AcceptsBackupQualityField`, `TestConfigRevision_LegacyWithoutField_StillWorks`, and `TestConfigRevision_MetricIncrements` pin the accept path, the legacy default, and the counter increment respectively. TriggerSource is unchanged on the wire (already `omitempty` would lose the explicit `poll` default; left as-is and confirmed end-to-end). Cross-repo follow-up: AUDIT-070 (X-Probe-Batch-ID handling) is still open.
