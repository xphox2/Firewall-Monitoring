...
### Fixed
- **Production crash-loop: the whole container restarted every few seconds and the web UI never came online (regression in v0.10.402; fixed v0.10.405).** The AUDIT-094 fail-fast entrypoint (v0.10.402) tore the **entire** stack down whenever *any* of the three daemons exited. That turned out to be too aggressive: the trap-receiver **refuses to start without an `SNMP_TRAP_COMMUNITY`** (AUDIT-012, to avoid an open trap listener), so on a deployment that doesn't set one it exits *immediately and forever* — and the fail-fast then crash-looped the container (`api` + web UI included) every ~3 seconds, so the API briefly served then got killed each cycle and never stayed online. Two-part fix:
  - **Entrypoint now supervises the *essential* service only.** Instead of `wait -n` (fire on the first daemon to exit), it does `wait "$API_PID"` — a `poller` or `trap` death leaves the **API/web UI up** (degraded but reachable, so the operator can actually see what's wrong) and only the **API** exiting tears the stack down for a clean restart. Verified with a mock harness: poller/trap dying at t=0 does not trigger teardown (the script blocks on the API), the API exiting → `exit 1`, and `SIGTERM` → graceful `exit 0`.
  - **Trap-receiver disables gracefully instead of fatal-exiting when no community is set.** Many deployments don't use a single global trap community (each firewall can have its own), so a missing `SNMP_TRAP_COMMUNITY` is a normal *"trap ingestion off"* state, not a fatal error. New `TrapReceiver.Enabled()` gates `cmd/trap-receiver`: with no community it logs one clear "trap ingestion DISABLED" line and **idles** (no open listener — AUDIT-012 still satisfied) rather than crashing, so the rest of the stack runs normally. Set `SNMP_TRAP_COMMUNITY` to enable trap reception.
  Net effect: the server comes online with **no required trap configuration**. Tests: `internal/snmp` `TestTrapReceiver_Enabled_AUDIT094`; `internal/shell/entrypoint_supervision_audit094_test.go` re-pointed at the essential-api design. **Requires a redeploy** (rebuilds the image entrypoint + the trap-receiver binary).
