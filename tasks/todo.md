...
- [ ] **Phase 2 (REDESIGN REQUIRED, then own plan-mode session):** deny detectors re-based on `syslog_messages` (`action=deny`, FortiGate key=value) instead of `flow_samples.firewall_event` — `deny_storm` (src-keyed; srcintf/dstintf replace direction-classification; WAN-inbound denies available from syslog without the WAN sampler), `deny_storm_victim` (victim-keyed), `denied_then_allowed` (syslog denies joined against flow_samples/syslog allows). Umbrella-plan Phase 2 section must be rewritten before implementation.
