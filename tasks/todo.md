...
- [x] Follow-up SHIPPED as PR #246 (v0.11.238): `Bot.Start` refuses an empty nick (releases the mutex, records "nick is empty", backs off) instead of panicking under `b.mu`; regression test `internal/irc/bot_empty_nick_test.go`; AUDIT-315 recover test now injects its panic via `startConnectHook`. The CI teardown timeouts were NOT a flake: `teardownConn` queued QUIT then `Disconnect` closed `end` before waiting, and the write loop's select dropped the QUIT at random when it was between writes — fixed by waiting (≤2 s) for the server to drop the session on `ErrorChan` before `Disconnect`; fake test server now closes on QUIT. 2 adversarial review rounds sound.
- [ ] Nit (pre-existing, out of scope): `bot_saslteardown_audit319_test.go` `serverStalled` deadline (25 s) exceeds its own 20 s watchdog, so the "no QUIT reached the server" branch can never be the reported failure.
