...
- [~] Batch 5 (Collector 1.3.39): implemented 9c34db5, pushed; review round 1 = SOUND-WITH-CHANGES (HIGH: netlink `re:` rate line clobbers stat: counters with zeros — reproduced; MED: PacketLoss==0 broken absence sentinel → healthy links got phantom loss from cumulative fallback; MED: vacuous T3 vdom fixture) — fixes dispatched to impl agent.
- [~] Batch 6 (Server 0.11.215): implemented a06eb58 (192 both leaks, 194/195+252 one hunk w/ empty-list⇒show-all, 260 delete-block removed, 199 409-guard, 262 both burn paths, 249 3-part + response-mask extension), pushed; fable review round 1 running (sharp axes: empty-ARRAY allowlist case, mask-before-save ordering).
