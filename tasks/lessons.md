...
## 2026-08-30 — Cache reads are not waste; the orchestrator's own context is. And Honcho memory is not on disk.

**Two corrections in one session.** (a) I reported the 2026-08-27 audit program's token profile as "96.45% cache reads" without framing it, and it read as waste. It is the opposite: modelling the bill against list prices reproduces the actual $1,900.56 within ~10%, and the same work with caching off costs **~$11,700**. Caching **saved ~$9,800 — 84% of the counterfactual**. Read:write is 35:1 and the median cache write is 1,496 tokens (just the newest turn), so there is no prefix churn either. Never present a cache-read share as a cost problem, and never "optimise" by reducing caching. (b) I claimed no written CTO Loop existed because `grep -ri "CTO loop"` over the filesystem found nothing. **Honcho conclusions are stored server-side.** Use `list_conclusions(layer='global')` — the loop was there all along.

**Where the money actually goes.** The cost is *context size × turn count*, and it concentrates in one place: the orchestrator thread. The remediation session ran a **median 491k-token context, p90 856k, max 999,798 — the 1M ceiling — across 1,709 turns**, and that single thread produced **~41% of all context replay** in the program. What filled it was not file contents (Read inputs were 0.9%) but its own tool calls: **Agent briefs 35.4%** (100 calls averaging 6,087 chars), **Bash 35.4%** (660 calls), **Edit+Write payloads 21%** (190 calls). Subagents ran at 105k median — 5x cheaper per turn. The fan-out was never the expensive part.

**Rules:** (a) The orchestrator is a dispatcher, not a worker — it must not open a file it is not about to edit itself, and implementation edits plus the QA loop belong in a subagent or worktree. (b) A subagent brief over ~40 lines goes in a file and is passed by path; brief text is permanent context, re-read on every later turn. Delegation only pays if the delegation itself is cheap. (c) Preview diffs with `--stat` and targeted greps, never by pulling whole diffs into context. (d) Never trade model or effort for cost — quality is not the lever. (e) Bound review by *what* is under review, never by round count: production code gets unbounded adversarial rounds and they earn it (5 code-only + 14 mixed review commits in this program caught real pre-merge bugs), but prose gets ONE accuracy pass plus a mechanical check. PR #243 ran **7 review rounds that touched zero production files** — the fix was one commit (21 files, −393 lines), then rounds 1-4 corrected changelog/ledger prose (round 2: *"Correct the correction — the replacement ledger claim was also false"*) and rounds 5-7 edited only `internal/shell/deadvpnwalk_audit320_test.go`, a guardrail test asserting that prose is accurate. Never put a test that guards prose into the review loop.

**Doctrine lives in Honcho, not here.** The amended CTO Loop (P1/P5/P6 rewritten to say *where* work happens, plus a new P7 review-termination rule) and a separate multi-agent-workflow authoring conclusion are in the global layer. The audit workflow script is authored fresh per run and is deliberately not hardened or version-controlled.

## 2026-09-15 — A test you cannot run locally must have its fixtures checked by hand. And when review keeps finding bugs in one mechanism, delete the mechanism.

**Two lessons from the Flows summary work (PR #259), which took five adversarial rounds.**

**(a) The PostgreSQL lane is the only place `//go:build integration` tests ever run, so a wrong ASSERTION there costs a full CI cycle and looks like a code bug.** I wrote `TestFlowSummaryIntegration_ExactAndEngineSafe` asserting port 443 was the top port, seeded 443 across two rows totalling 1,700 bytes and port 53 with a single 2,000-byte row, and CI correctly reported "top port value is 53, want 443". The code was right; I had not added up my own fixture. **Rule: for any test that cannot run on the dev machine, do the fixture arithmetic explicitly before pushing, and prefer magnitudes that make the intended winner unambiguous (90,000 vs 2,000, not 1,700 vs 2,000).** Ambiguous fixtures make a passing test luck.

**(b) Successive review rounds finding bugs in the SAME mechanism is a design signal, not bad luck.** The summariser's change-detection went: redo-window → id watermark → watermark + dirty cursor → cursor + pinned epoch ceiling → contiguous cursor. Rounds 3 and 4 each found a fresh silent-staleness hole in that bookkeeping, and every fix opened the next one: a bucket re-dirtied behind the cursor was skipped then buried; a bucket that FAILED inside a truncated walk dropped below the cursor and was never retried; an unpinned epoch ceiling buried mid-pass arrivals. Three data-loss bugs from one piece of state.

The cursor existed only so a truncated walk could resume. **Removing the truncation removed the cursor, the epoch ceiling and all three bugs — 239 net lines.** The walk over CHANGED buckets now always completes; only the BACKFILL is time-bounded, which is safe because its contiguous fill marker is resumable without skipping. It is affordable because the dirty list is bounded by what changed since the last pass, not by history.
